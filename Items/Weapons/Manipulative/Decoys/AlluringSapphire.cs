...
{
    class AlluringSapphire : DecoyItem
    {
        public override int DecoyType => ModContent.NPCType<AlluringSapphireDecoy>();
        public override int CooldownSeconds => 35;
        public override string DecoyName => "alluring sapphire";
        public override int DefaultHP => 100;
                ...
                if (NPC.ai[0] == Main.myPlayer && Main.netMode != NetmodeID.Server)
                {
                    for (int n = 0; n < Main.rand.Next(3, 5); n++)
                    {
                        int p = Projectile.NewProjectile(new EntitySource_OnHurt(NPC, null),
                            ...
                            ModContent.ProjectileType<SapphireSoul>(),
                            (int)(NPC.damage * 2f),
                            0f,
                            (int)NPC.ai[0]);
    ...
    class SapphireSoul : ModProjectile
    {
        public override string Texture => "Wisplantern/Items/Weapons/Manipulative/Decoys/AlluringSapphire";
        ...
        public override void AI()
        {
            Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.BlueTorch, Projectile.velocity.X, Projectile.velocity.Y);
