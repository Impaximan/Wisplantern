...
{
    class AlluringTopaz : DecoyItem
    {
        public override int DecoyType => ModContent.NPCType<AlluringTopazDecoy>();
        public override int CooldownSeconds => 35;
        public override string DecoyName => "alluring topaz";
        public override int DefaultHP => 75;
                ...
                if (NPC.ai[0] == Main.myPlayer && Main.netMode != NetmodeID.Server)
                {
                    for (int n = 0; n < Main.rand.Next(3, 5); n++)
                    {
                        int p = Projectile.NewProjectile(new EntitySource_OnHurt(NPC, null),
                            ...
                            ModContent.ProjectileType<TopazSoul>(),
                            (int)(NPC.damage * 2f),
                            0f,
                            (int)NPC.ai[0]);
    ...
    class TopazSoul : ModProjectile
    {
        ...
        public override void AI()
        {
            Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.YellowTorch, Projectile.velocity.X, Projectile.velocity.Y);
